...
2. Verify every selected property actually belongs to `<Type>` (or one of its ancestors):
   ...
   Each `--property` value goes through its own quote-delimited heredoc — these are candidate names this step itself just proposed, not values an earlier script already verified. Drop any property the script reports as `ERROR:` from the candidate list — **never** put an unverified property into the new schema file's `properties:` block. If every candidate property fails verification, proceed with an empty `properties:` block rather than skipping the proposal (the type itself is still verified as real; missing property suggestions is not a blocker for opening the PR — reviewers can add fields by hand).
3. Generate `.wikicommit/schema/<Type>.md` in the standard-type format (Issue #495's `properties:`-nested layout). Use `.wikicommit/schema/default.md` (frontmatter skeleton) and `.wikicommit/schema/Person.md` (a concrete, already-standard example) as **fixed** style references — do not attempt dynamic "closest existing type" selection:
   - `wikicommit.base: https://schema.org/<Type>`
   - `wikicommit.provenance: schema-propose` (do not copy `Person.md`'s own `provenance: default` value — each write site stamps its own origin; applies uniformly to both the standard-type and custom-type paths of this Skill, see Step 5 below)
   - `wikicommit.granularity:` 1–3 rules in the same style as `Person.md`/`DefinedTerm.md`, grounded in the observed example page(s). If a property that would technically apply to `<Type>` should deliberately not be recorded (the old `excluded` field's role), state that as a granularity rule in prose instead (e.g. "Do not record `<property>` even when the source states it — out of scope for this wiki's purpose"), rather than declining to verify/list it silently. Add one rule starting with `Boundary` saying what the type is *not* for, the convention every distributed base type follows (Issue #550). **Where another installed type is the better home for a recognizable class of subject, say so as its own `granularity` rule and name that type** — e.g. `Prefer schema:HowTo when the source's substance is an ordered set of steps the reader performs`. `granularity` is where cross-type deference lives (there is no separate field for it), and `wikicommit-generate` Pass 2c is instructed to follow such a line over its own read of the fit (Issue #569). It is not the same as the `Boundary` rule: `Boundary` says what the type is not, this says who should have it instead. Name only types that are actually installed — a line pointing at a type this wiki does not have cannot be acted on. Never write a rule that contradicts a rule stated elsewhere in the pipeline — in particular `wikicommit-generate` Pass 2a's source-as-entity test, which admits a source with a fixed publication date and authors and excludes a continuously-updated living resource (an official document, a government procedure page, a Wikipedia article). A `granularity` that says such a resource qualifies for a page of its own contradicts it, and no Skill can edit the file afterwards to take it back (Issue #552). Every bullet has to survive YAML parsing as a plain string: write the boundary rule `Boundary — …` with an em dash rather than `Boundary: …` (a `": "` in an unquoted list item turns the bullet into a one-key mapping that every `isinstance(g, str)` consumer skips), and keep ` #` out of the middle of an unquoted bullet (it opens a YAML comment and truncates the line) — wrap the whole bullet in double quotes if the wording needs either character.
