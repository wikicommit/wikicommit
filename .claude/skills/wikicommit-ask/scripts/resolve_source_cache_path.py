...
Reads the URL from stdin (not argv — sources[].url is only format-validated
by validate_frontmatter.py, never verified safe for shell-argument use). Prints the cache file path
(.wikicommit/.cache/ingest-fetch/<scratch-path>.md, repo-root-relative) on
stdout if a matching source management file is found AND the corresponding
cache file exists on disk. Otherwise prints nothing.

Exit codes:
    0 — cache file found and printed
    1 — no matching source management file, or the cache file doesn't exist
"""
