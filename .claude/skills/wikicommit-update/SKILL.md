...
**`quartz.config.yaml`** — show the new keys and where they sit in the file. **Do not present the repository's own settings as differences**: `pageTitle`, `pageTitleSuffix`, `baseUrl`, `links`, `theme` and anything under `translation` are this wiki's, and listing them as drift buries the one line that actually matters. Apply what the user accepts by hand, in place, preserving their formatting and comments.
