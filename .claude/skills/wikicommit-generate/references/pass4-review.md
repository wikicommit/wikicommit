...
1. Launch a subagent and give it exactly these five things, and nothing else:
   ...
   **An empty result is a normal answer**, not a failure: `SUMMARY: outbound=0, inbound=0` means this page has no neighbours yet. Check that a `SUMMARY:` line was printed at all — its absence means the script did not run, which is a different thing entirely.
   ...
   This used to be prose, and it described only the inbound half; the outbound half said just "the existing pages this page links to". A run filled that gap with an improvised regex whose character class had no `-`, which matches none of this wiki's kebab-case slugs (Issue #193) — and the failure was silent, because a smaller set looks exactly like a smaller neighbourhood (Issue #947).
