...
- **Do not extract a document this source only cites in passing (Issue #968).** When the source names another document once — a contrastive citation, a line in a related-work paragraph, an entry in the reference list — while writing about something else, that document is not an entity. Leave it out of `entities` entirely. **The opposite case does extract**: where the source treats that document, or the fact it is cited for, as *its own subject* — its title, or a central claim, is about it — it is an ordinary entity and the rest of the rules here apply to it unchanged. Both halves are written here on purpose: a boundary stated on one side only gets applied on one side only (Issue #550's lesson about `granularity`, one stage earlier).
  ...
  **The test is the same one `.wikicommit/review-rules.md` uses**, applied one stage earlier: does this source treat that document's subject as its own subject, or does it name it once while writing about something else? **Where you cannot tell, treat it as the passing mention and do not extract.** That tie-break is not a preference — check 4 in the review rules breaks the same tie the same way, and a Pass 2c that were more permissive than the review at exactly the unclear cases would guarantee the wasted round: extract, write, fail, record, repeat.
  ...
  Why it belongs here and not only downstream: the rule already existed at the two later stages — Pass 3 will not write another document's date or title as established fact (`references/pass3-generate.md`, Secondary citation discipline), and Pass 4 fails the page with `MISSING_SOURCE`. Both sit *after* the entity has been cut, so the page gets written and then thrown away. In `wikicommit/ai-driven-dev-wiki` one arXiv paper's contrastive citation of another produced a `ScholarlyArticle` page that the review discarded with exactly that finding — and because the failure is recorded in `failed_pages`, the source keeps matching Pass 1's collection condition (`partial` with a non-empty `failed_pages`), so every later run cuts the same entity, writes the same page and discards it again. Nothing about the inputs changes between runs, so nothing about the outcome does either.
