...
export default defineConfig({
  ...
  // Only what JSX compilation actually needs bundled — never [/.*/] (what
  // upstream uses), which silently defeats every entry in SINGLETON_EXTERNALS
  // (Issue #399). The inline-script loader below is unaffected: it runs its
  // own isolated esbuild.build() with its own bundle/external options.
