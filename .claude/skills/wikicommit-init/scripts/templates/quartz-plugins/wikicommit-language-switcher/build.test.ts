...
describe("built dist/index.js (Issue #399)", () => {
  it("does not bare-import preact and still emits the language switcher's own markup", () => {
    const outDir = mkdtempSync(join(tmpdir(), "wikicommit-language-switcher-build-"))
    try {
      ...
      expect(built).toContain("wikicommit-language-switcher__label")
    } finally {
