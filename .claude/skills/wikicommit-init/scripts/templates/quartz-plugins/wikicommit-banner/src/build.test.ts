...
describe("built dist/index.js (Issue #379)", () => {
  it("does not bare-import preact and still emits the banner's own markup", () => {
    const outDir = mkdtempSync(join(tmpdir(), "wikicommit-banner-build-"))
    try {
      ...
      expect(built).toContain("wikicommit-banner__report")
    } finally {
