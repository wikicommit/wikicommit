...
describe("WikiCommitBanner", () => {
  ...
  it("renders the pending banner when review_status is pending", () => {
    ...
    expect(html).toContain("Nobody has read this page yet")
  })
  ...
  it("renders only the report link on a reviewed page that records no generation", () => {
    ...
    expect(html).not.toContain("Automatically generated by an LLM")
  })
  ...
  describe("a reviewed page keeps saying it was generated", () => {
    ...
    it("still states that the page was written by a model", () => {
      ...
      expect(html).toContain("Automatically generated by an LLM")
    ...
    it("says the same thing a pending page says, word for word", () => {
      ...
      const body = "Automatically generated by an LLM. It may contain inaccuracies."
    ...
    it("stays visually distinguishable from a pending page", () => {
      ...
      // which is the whole point of showing `reviewed` at all. Three signals
      // carry it: the modifier class (border colour), the icon and the heading.
      const pending = renderBanner({ ...reviewedPage, review_status: "pending" })
      const reviewed = renderBanner(reviewedPage)
      expect(pending).toContain("wikicommit-banner--pending")
      expect(reviewed).toContain("wikicommit-banner--reviewed")
      expect(pending).toContain("Nobody has read this page yet")
      expect(reviewed).toContain("A person has read this page")
    })

    it("keeps the warning icon for the pending state only", () => {
      // ⚠️ belongs to "nobody has read this yet", which is the one part that
      // really does end at review — not to "written by a model", which does not.
      expect(renderBanner({ ...reviewedPage, review_status: "pending" })).toContain("⚠️")
      expect(renderBanner(reviewedPage)).not.toContain("⚠️")
    })
    ...
    it("carries the fact through the heading when no reviewer was recorded", () => {
      // Route B pages and anything reviewed before `reviewed_by` existed have
      // no name. The heading already says a person checked the page, so no
      // name-less placeholder line is added (Issue #663's rule that a missing
      // reviewer is a normal state, not a gap, is unchanged).
      const html = renderBanner(reviewedPage)
      expect(html).toContain("A person has read this page")
      expect(html).not.toContain("Read by:")
  ...
  it("falls back to cfg.locale when frontmatter.lang is absent", () => {
    ...
    expect(html).toContain("このページはまだ誰も読んでいません")
  })
