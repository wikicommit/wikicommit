...
describe("built dist/index.js (Issue #399)", () => {
  it("does not bare-import preact and still emits the breadcrumbs' own markup", () => {
    const outDir = mkdtempSync(join(tmpdir(), "wikicommit-breadcrumbs-build-"))
    try {
      ...
      expect(built).toContain("breadcrumb-container")
    } finally {
