...
> **Quartz サブモジュールの更新手順は `.wikicommit/guides/updating-the-quartz-submodule.md` に置く（Issue #988）**: `quartz/` は他人のリポジトリであり親が持つのはコミットの参照 1 行だけだが、WikiCommit のビルドはその中へ書き込む — `quartz/content`・`quartz/quartz.config.yaml`（`prebuild-symlinks.cjs`）と `quartz/quartz.lock.json`・`quartz/.quartz/plugins/*`（`npm run install-plugins`）は未追跡で git 操作を止めないが、**`postinstall` が `quartz/` 内で走らせる `npm install` による追跡ファイル `quartz/package-lock.json` の変更**だけは `quartz/` 内の `git pull` を必ず止める。更新手順はそれまでどこにも無く、唯一の関連記述（`print_next_steps.py` の `_PREVIEW_STEP`）は Windows-only の段落にあり、未追跡ファイルしか見ず、「nothing to clean up」と断言していた。`_PREVIEW_STEP` は直した（全プラットフォームの段落に出し、`package-lock.json` を含め、ガイドを指す）が、あの文面は init のときにしか印字されず既存リポジトリに届かないため、**届く経路はガイド棚（`update: overwrite`。再 init と `/wikicommit-update` で届く）が担う** — Issue #867 がエンティティポリシーのガイドを棚に置いたのと同じ非対称である。ガイドは `(cd quartz && npm install)` を明示する: ルートの `npm install` は `postinstall` の `git submodule update` で、`git add quartz` 前の古いポインタへ黙って巻き戻すため。
>
> **採らなかった案**: (a) `.gitmodules` に `ignore = dirty` を案内する — 主題（更新が塞がれていること）とは別の表示の問いであり、しかも主訴だった VSCode の「↓7」表示（エディタがサブモジュールを別リポジトリとして upstream と比較している）には効かない。(b) `quartz/` 内を `npm ci` にして汚さない — `package.json` と lockfile がずれていれば hard failure になり、ずれは上流 Quartz と利用者の npm の版で決まってこちらが制御できない。「黙って汚れる（更新しようとした人だけが困る）」を「ビルドが通らない（全員が困る）」に交換することになり、Quartz のビルドはこのプロジェクトが最も焼かれてきた領域である。(c) `.vscode/settings.json` を配る — 利用者のエディタ設定であり、WikiCommit は `.vscode/` に何も配っていない。**dev2 では検証できない**（Issue #81 で Quartz 一式を削除済み）ため、ガイドの手順は標準的な git / npm 操作だけで構成した。`CHANGELOG.md` は更新していない（型テンプレート・ページ生成ルールの変更ではない）。
...
>
> **公開サイトで確認済み（Issue #657。2026-09-10）**。このリポジトリには Quartz 本体が無い（Issue #81）ため、確認は 3 つのパイロットの公開サイトで行った — 修正前の `quartz.config.yaml` を持つ 2 件（`wikicommit/saitama-city-wiki` / `wikicommit/decameron-wiki`）と、0.5.0 で新規 init した 1 件（`wikicommit/ai-driven-dev-wiki`）である。前者は `always_skip_existing` により `exclude` が届いていないので、**同じ機能の before / after がいま同時に公開されている**。
>
> | | 修正前（saitama / decameron） | 修正後（ai-driven-dev-wiki） |
> |---|---|---|
> | フォルダページ（`/ja/`・`/en/`） | `wikicommit-banner--pending` + 報告リンク | **どちらも出ない** |
> | タグページ（`/tags/`） | 同上 | **どちらも出ない** |
> | 通常のコンテンツページ | `wikicommit-banner--pending` | 変わらず出る |
> | root `content/index.md` | `wikicommit-banner__report`（サイト集計付き） | 変わらず出る |
>
> Issue #648 が前提とした症状（フォルダ・タグページにバナーと報告リンクが出る）は saitama で実際に確認でき、`exclude` 追加後にその 2 種だけが消え、通常ページと root index は巻き添えを受けていない。**したがって案 1（レイアウト設定から外す）でよかったことが実データで裏づけられた** — 案 2（コンポーネント側でページ種別を判定する）へ切り替える必要は無い。なお decameron 側はビルドが Issue #245 / #742 より前で報告リンク自体を持たないため、バナーのみの確認になる。
>
> **Type 別インデックスは Quartz の「フォルダページ」として描画される**（同 Issue の 4 点目）。`/en/definedterm/` は `rebuild_index.py` が書いた `index.md` の WikiLink 一覧と、Quartz が自動生成する「17 items under this folder」の一覧を**同居させて**表示する。つまり `index.md` があってもフォルダページが抑止されるわけではなく、その本文としてフォルダページに取り込まれる。
>
> **この 1 種については 2 つの機構が層になっており、どちらも消してはならない**:
>
> | | 効く範囲 | 届き方 |
> |---|---|---|
> | `review_status: reviewed` のスタンプ（#580） | 全 `wikicommit-banner` を `wikicommit-banner__report` へ落とす | `rebuild_index.py` ＝ `.wikicommit/scripts/` ＝ `update: overwrite`。**既存リポジトリにも届く** |
> | `layout.byPageType.folder.exclude`（#648） | コンポーネント自体を描画しない | `quartz.config.yaml` ＝ `always_skip_existing`。**既存リポジトリには届かない** |
>
> 新規 init では `exclude` が先に効くのでスタンプは重複になるが、**`exclude` が届かない既存リポジトリではスタンプだけが効いている** — 実際 saitama の `/ja/place/` は `wikicommit-banner__report` を出しており（プレーンなフォルダページ `/ja/` が `--pending` を出しているのと対照的である）、そこでバナーを黙らせているのはスタンプの側である。Issue #580 のスタンプを「`exclude` があるから不要」として削ると、**既に公開されている全リポジトリの Type 別インデックスにフルバナーが復活する。**
>
> `tests/test_template_mirror_sync.py` が配布テンプレートの `exclude` に `wikicommit-banner` が入っていることを CI で検証し、設定側の回帰を止める。
>
...
**「タグのままで十分か、ページ化すべきか」の暫定判断基準**（Issue #275 が扱う「付与ルール」とは別軸。「タグに退避したまま実体ページを持たない概念をどう扱うか」という問いへの回答。問いの出どころは `dev/issue_candidate.md`）:
...
**素の `github:quartz-community/graph` では対応できない**。`D3Config` が持つのは `drag`/`zoom`/`depth`/`scale`/`repelForce`/`centerForce`/`linkDistance`/`fontSize`/`opacityScale`/`removeTags`/`showTags`/`focusOnHover`/`enableRadial` がすべてで、ページ単位の除外・言語・型・次数のフィルタをいずれも持たない（`removeTags` はタグ疑似ノードを消すのみでページノードには効かない — フォークは Issue #982 でこの 2 キーを `filterNodes()` 側へ移した。§8.7.1）。しかもこれらはビルド時に `data-cfg` 属性へ JSON として焼き込まれ、UI から変更する手段がない。
...
**808行の `graph.inline.ts` には D3 force simulation と PixiJS の描画本体がある。フィルタの動的化はそこに一切触れずに実現できた**（描画本体への変更は後から 2 件入った — 下記 Issue #841 と Issue #986）。動的化に必要な土台が既に揃っていたため:
...
上流からの変更はソース中に `WikiCommit:` として印を付けてある。**当初の 4 箇所はいずれも描画本体の外側**（設定の読み取り・`neighbourhood` の絞り込み・コントロールバーの構築・クリック除外リスト）であり、その後 4 件が加わった: 凡例（5）・**ノードの形による種別の区別（6。Issue #841）**・タグ疑似リンクの 2 分岐（7。Issue #982）・**ラベルの不透明度の一本化（8。Issue #986）**。**描画本体に触れているのは太字の 2 件だけである**（7 はリンク構築、5 はバーの構築であり、どちらも外側）。

#### 種別は形、訪問状態は色（Issue #841）

**source ノードが entity ノードと完全に同じ描画だった**。upstream が区別していたのは tag（中空の円 + `--tertiary` の 2px リング）と、それ以外（塗りつぶした円）の 2 つだけであり、`globalGraph.showSources` の既定が `true` である以上、`content/sources/` の導入（Issue #476）以降は **source がノードの最大勢力でありながら見分けが付かない**状態だった。しかも「中空＝タグ」という既にある規約をどこにも説明していなかった。
...
そこで **upstream が tag に対して既に採っている分担（種別＝形）をそのまま延長し、source を四角にした**。色の割り当ては一切変更していない。色覚に依存せず、既存の訪問状態エンコードを壊さず、分岐は既にその場所にある。副次的に**分類ロジックの二重実装が 1 つ消えた** — 描画側の `nodeId.startsWith("tags/")` は、同じファイルが既に `filterNodes()` / `collectFacets()` を取っている `nodeFilter` の `classifyNode()` をその場で書き直したものであり、**source の case を持たなかったのがまさにこの欠陥の成り立ちである**（`classifyNode` 自体は import されていなかったため、置き換えにあたって import に加える必要がある — この 1 行を落とすと、`@ts-nocheck` と eslint の除外対象であることが重なって型検査にも lint にもビルドにも掛からず、ブラウザが最初のノードを描く時点で `ReferenceError` になる）。四角の半辺は半径の 0.9 倍にしてある（半辺＝半径だと面積が円の 4/π 倍になり、隣の円より明らかに重く見えるため）。

**型（20 種）は形にも色にも載せない**。区別できる表現が無く、載せれば凡例が 20 行になる。型の絞り込みはコントロールバーの型フィルタが既に担当しており、そちらが「見たい型だけにする」正しい手段である。

**凡例をコントロールバーに置いた**。形 3 種（ページ / タグ / 情報源）と色 3 種（現在のページ / 訪問済み / 未訪問）の両方を説明する — どちらも graph を見ただけでは推測できず、対で読むものだからである。スワッチは graph と同じテーマ変数・同じ形で描くので、片方だけがスタイル変更で置き去りになることが構造的に起きない。**形の側のスワッチだけは `--darkgray` を使う** — 3 つの状態色のどれでもないため「4 つ目の状態」と読まれず、かつ「ページ」と「未訪問」が同じ灰色の円にならない（初版はそうなっており、2 つの軸がそれぞれの最も素朴な項目で衝突していた）。2 つの軸の間には改行を入れ、6 項目の 1 列ではなく 2 群として読めるようにしてある。ラベルは既存の `data-labels` 経路にそのまま乗り、`sources` / `tags` は 30 ロケールに既にある訳を再利用して、新規キーは 5 つ（`legend` / `legendPages` / `legendCurrent` / `legendVisited` / `legendUnvisited`）に留めた。`i18n/index.ts` の `Record<string, typeof enUS>` が全ロケールへの追加を型で強制する。

**採らなかった案**: 型名からハッシュで色相を振る（テーマ外の色が混ざり、20 種は色相で区別できず、色覚にも依存する）／色を種別へ付け替えて訪問状態を捨てる（訪問済みの可視化は upstream の機能であり、グラフを回遊の手掛かりにしている読者から取り上げることになる）／凡例の項目をそのまま Sources / Tags トグルと統合する（魅力的だが、トグルの形が決まってからでないと設計できない。凡例を置くところまでに留めた）。

**ブラウザでの目視確認は行っていない**（このリポジトリに Quartz 本体が無い。Issue #81）。機械的に検証してあるのは、分類が `classifyNode()` 一本になったこと・source が `rect` で描かれること・全ロケールが凡例ラベルを持つこと・スワッチがテーマ変数のみを使い訪問状態側が円のままであること（`test/legend.test.ts`）に限られる。

> **コントロールが自分の意味を説明していなかった 2 点（Issue #984）**: 上の 2 節が作ったもの — 次数フィルタ（Issue #584）と凡例（Issue #841）— は、どちらも**読者が読んで意味が取れる状態になっていなかった**。片方はラベルの欠落、もう片方は**ラベルが実装に対して端的に偽**だった。
>
> **(1) 次数の 2 入力は、どちらが下限か分からなかった。** `buildDegreeRange()` は 1 つのキャプション（「ノードあたりのリンク数」）の下に区別のつかない数値入力を 2 つ並べるだけで、手掛かりは `minInput.title = "0 = no lower bound"` というホバーしないと出ない文字列、しかも**ハードコードされた英語**（30 ロケールに訳のある他のラベルと違い、日本語 Wiki でも英語で出る）だった。**同じ問題は §8.7.1 自身が別の文脈で既に記録していた** — Issue #839 の prune を導入したとき「`minDegree: 1` でも孤立点は消えるが、ラベルが "Links per node" であるためこの用途に使えると気づく手段が無く」と書いており、そのときは別の機構で回避してコントロール自体は変えていない。
>
> 現在は各入力が自分のキャプション（`degreeMin` / `degreeMax`）を持つ。**ラッパーは `<label>` から `<div>` に変えた** — 各入力が自前の `<label>` を持つ以上、`<label>` の入れ子になるためである（CSS はクラスで選択しているのでこの変更は見えない）。**縦に積まず横に並べる**のは、バーが流しで伸びる設計（Issue #838）の下では 2 行目がこの 1 フィールドだけを他より高くするためである。`title` は残すが、**残す内容は「0 は境界なし」1 つだけ**になった — どちらの端かは画面上のキャプションが言うので、`degreeNoBound` という共有の 1 キーで足りる。0 の意味を見える位置に出す案は採らなかった（キャプションが 2 倍の長さになり、バーの幅を上の理由と同じだけ食う）。
>
> **`sync()` のキャレット保護には触れていない。** 値の書き戻しが「文字列が実際に異なるときだけ書く」のは、`change` が Enter で発火したときにフィールドがフォーカスされたままキャレットが飛ぶのを避けるためであり（Issue #651 の `{field, sync}` 契約）、本件が変えたのは**入力のラベル付けだけ**である。テストがこの 2 行を固定している。
>
> **(2) 凡例の「訪問済み」は、タグに対しては嘘だった。** 描画側の色の分岐は `visited.has(d.id) || classifyNode(d.id).kind === "tag"` であり、**タグノードは訪問の有無に関わらず常に `--tertiary`** — 凡例が「訪問済み」と説明している色 — で描かれる。これは upstream から引き継いだ挙動だが、**凡例を置いたのは Issue #841 である**: ラベルが後から来て、その例外に気づかないまま名前を付けた。結果として Tags を on にしている読者の画面では、一度も開いたことのない 100 件以上のタグが「訪問済み」の色で並ぶ。
>
> **挙動ではなく凡例を直した。** タグから色の例外を外す案（`|| classifyNode(d.id).kind === "tag"` を落とす）は、凡例の文言を揃えるためだけに upstream の意図（タグは回遊の手掛かりであって訪問対象ではない）を壊す — Issue #841 が「色の割り当ては一切変更していない」と決めたのと同じ理由で採らない。
>
> **(3) 「訪問済み」の定義はどこにも書かれていなかった。** 実体は `localStorage` の `graph-visited` キーで、述べているのは「**このブラウザでそのページを開いたことがあるか**」だけである。次のいずれでもない: セッション単位ではない（タブを閉じても残る）／サーバ側の記録ではない（他の読者の訪問は反映されない）／`review_status` とは無関係（`reviewed` は「人が読んだ」という Wiki の記録〈Issue #800〉であり、こちらは「このブラウザで開いた」でしかない）／端末・ブラウザをまたがない。**この 4 つの否定はどれも読者が誤解しうる方向である。**
>
> **(2) と (3) はどちらも `title` 属性で補う。** 凡例に散文を足すと 6 項目 2 群という形（Issue #841）が壊れ、バーの高さがさらに増える（Issue #838 が触った制約）。キーは `legendTagsAlways` / `legendVisitedHint` の 2 つで、次数側の 3 つと合わせて 5 キーを 30 ロケールに追加した（`i18n/index.ts` の `Record<string, typeof enUS>` が追加漏れを型で止める）。**これらは読者向けの文字列なので翻訳する** — Issue #770 が確立した「読み手で決める」線引きに従えば、コントロールバーは公開サイトの UI であり読者が読む側に属する。
>
> **ヒントの `0` は ASCII のまま書く。** `input.value` は全ロケールで `String(0)` なので、ローカライズした数字（実際に混入したのはペルシア語の `۰`）を使うと、**説明している入力欄に決して現れない文字**を名指すことになる。テストが全ロケールについてこれを固定する。
>
> **既知の限界: `title` はホバーしないと出ず、スクリーンリーダーでの読み上げも実装依存である。** (1) で「ホバーが問題の本体」としてキャプションを可視にした一方、(2)(3) を `title` に置くのは一貫しないようにも見えるが、分かれ目は**そのコントロールを操作するのに要るか**である — 下限か上限かは入力する前に知る必要があるのに対し、タグの色の例外と `graph-visited` の定義は読み解くときの補足であり、バーの形を崩してまで常時表示する価値が無い。`aria-label` は可視ラベルを置き換えてしまうため使えず、`aria-description` は対応が広くない。
>
> **`graph-visited` を `review_status` に結び付ける案は採らない**（別の軸であり、混ぜると `reviewed` の意味が弱まる）。**2 入力を 1 つのレンジスライダーにする案も採らない** — 上限は「ハブを隠す」ために使われる以上、214 本のようなハブを切るのに具体的な数値を打てることに意味がある。
>
> **ブラウザでの目視確認は行っていない**（Issue #81）。機械的に固定してあるのは、両ヒントが `labels` 経路に乗っていること・ハードコード英語が残っていないこと・5 キーが 30 ロケールに揃っていること・キャレット保護が無変更であること・`<label>` の入れ子が無いことである（`test/legend.test.ts`）。**遡及適用は行わない**（`quartz-plugins/` は `update: overwrite`）。

#### Sources / Tags の印はトグルが持つ（Issue #985）

> **凡例とトグルの合流 — 案 A（スワッチをトグル側へ移す）を採った**: 上の「採らなかった案」が保留した統合である。前提条件だった「トグルの形」は Issue #840 で `aria-pressed` の 2 状態ボタンに決まり、Tags トグルが実際にノードを消すことも Issue #982 で揃った。それまでのバーは **「情報源」「タグ」を 2 回ずつ**表示していた — 中ほどの押せるトグルと、末尾の押せない凡例の行である。
>
> **採った形**: `buildToggle()` に任意の 4 引数目 `swatch`（`{modifier, hint}`）を足し、Sources トグルには四角、Tags トグルには中空円のスワッチをボタンの中に置く。凡例からは「タグ」「情報源」の 2 行を落とし、「ページ」＋色 3 種の 4 項目になる。スワッチは**凡例と同じクラス**（`global-graph-controls__legend-swatch--*`）を使うので、2 つの印がスタイル変更で食い違うことは構造的に起きず、凡例のスワッチと同じく `aria-hidden="true"` でスクリーンリーダーにはラベルだけが読まれる。タグ色の例外を述べるヒント（`legendTagsAlways`。Issue #984）は、タグの印と一緒に Tags トグルの `title` へ移した（キー名は 30 ロケールの互換のため据え置く）。押された状態の塗り（`--secondary`）の上では `--darkgray` の四角がライトモードで沈むため、押下中だけ `--light` の 1px リングを付ける — ラベルが既に使っている色であり、テーマで同じ向きに反転する。
>
> **この形を採る理由は §8.7.1 が既に書いている原則の裏返しである**: トグルの状態をグラフ側で伝える案を「状態はコントロール自身が持つべき」として退けた以上、**そのトグルが何を出し入れするかの説明もコントロールの隣に属する**。得られるものは 4 つ — 同じ語が 1 度しか出ない・トグルが自己説明的になる（「情報源」という語だけでは四角いノードのことだと分からなかった）・**凡例が非対話のまま保たれる**・バーが短くなる。`{field, sync}` 契約は変えていないので、Issue #651 の書き戻し経路はこの変更を知らない。
>
> **凡例の「ページ」の行は残す**。ページには対応するトグルが無い — off にすると entity ノードが全て消え、それに繋がっていた tag / source も Issue #839 の prune で道連れになり、実質「グラフを空にする」ボタンになる。印の置き場が凡例にしか無い。**トグルを off にしてもスワッチは消えない**（ボタンの塗りが消えるだけ）ので、「四角 ＝ 情報源」の説明は状態に関わらず残る。
>
> **採らなかった案 — 凡例の項目自体をボタンにする（案 B）**: 凡例 6 項目のうち 2 つだけが押せる状態になり、押せない側のうち「ページ」は見た目が最も近いので**押せそうに見えて押せない**。色の 3 項目にはトグルという概念自体が無い。「凡例は説明・コントロールは操作」という読者が最初に立てる仮説を、部分的にだけ裏切る形である。
>
> **形を持たないトグルにはスワッチを付けない**。索引ページ（Issue #983）は円で描かれ entity と見分けが付かないので、仮にトグルを足しても出す印が無い。スワッチのあるトグルと無いトグルが並ぶことになるが、それは正直な非対称である（形で見分けられないものに形は出せない）。**本件は索引トグル自体は足していない** — 下の Issue #983 のコールアウトが露出を本件に委ねていたが、本件の範囲は既存 2 トグルと凡例の重複解消に留め、露出は Issue #1018 に切り出した。
>
> 機械的に検証してあるのは、凡例からタグ・情報源の行が消えたこと・凡例が非対話のままであること・トグル内のスワッチが凡例と同じクラスで `aria-hidden` であること・押下中のリングがテーマ変数のみを使うこと（`test/legend.test.ts`）に限られる。**ブラウザでの目視確認は行っていない**（上と同じ理由）。

#### ラベルの不透明度はホバーが先・ズームが後（Issue #986）

969 ノード規模（`wikicommit/ai-driven-dev-wiki` の実測）のグローバルグラフで、**既定の倍率ではラベルが 1 つも出ず、読める倍率まで寄せると全ノードのラベルが一斉に出て判読できなくなる**。そしてホバーしても、繋がっている相手の名前は読めない — ノードとリンクは強調されるのに、ラベルだけが上がらない。

**原因は 3 つの機構が別々に `label.alpha` を書いていたことである。** (1) ラベルは alpha 0 で生成される。(2) ズームハンドラだけが、倍率から導いた 1 つの値を**active でない全ラベルへ**代入する（active なものは**飛ばす**だけで 1 に上げてはいないので、直前のズームが入れた値のまま据え置かれる）。(3) ホバー経路は**ホバー中のノード自身のラベルしか**上げない。

| 状態 | ホバー中のノード | 近傍 | それ以外 |
|---|---|---|---|
| 既定倍率・ホバーなし | 0 | 0 | 0 |
| 既定倍率・ホバー中 | **1** | **0** | 0 |
| 寄せた状態・ホバー中 | 1 | `scaleOpacity` | `scaleOpacity` |

**近傍とそれ以外がラベル上で一度も区別されない。** `updateHoverInfo()` が立てる `active` を `renderNodes()`（1 / 0.2）と `renderLinks()`（1 / 0.2）は最初から読んでいるのに、`renderLabels()` だけがその分岐に参加していなかった。

**現在は `renderLabels()` が唯一の書き手である。** 優先順位はホバーが先・ズームが後で、判定そのものは `src/util/labelOpacity.ts` にある（`nodeFilter.ts` / `controlBar.ts` を切り出したのと同じ理由 — inline スクリプトは `@ts-nocheck` かつビルド時に esbuild ローダーが組み立てるため、中身は型検査も単体テストもできない。**このリポジトリではグラフを目視できない**〈Issue #81〉以上、真理値表を直接 assert できることが唯一の検証手段になる）。
...
**ズームハンドラは `renderLabels()` を呼ぶだけになった。** 置き換えた実装はラベルコンテナを走査しながら各要素について active なラベルの配列を `.indexOf()` で探しており、969 ノードではズームやパンのたびに約 100 万回の比較が走っていた。現在は 1 回の線形走査である。副次的に、**ホバー中にズームしてもホバーが描いた区別が消えなくなる**。

**`pointerover` / `pointerleave` の alpha の退避・復元も落とした。** あれは「ホバー中のラベルを誰も下げ戻さない」ことへの対処だったが、`renderLabels()` が毎回すべてを導出する以上不要であり、しかも**到達不能だった** — 復元の次の行の `renderPixiFromD3()` が即座に上書きしていた。残すと `label.alpha` の書き手が 3 つ目として残り、それはこの変更が消そうとしている形そのものである。
...
**ブラウザでの目視確認は行っていない**（Issue #81）。機械的に固定してあるのは、真理値表（ホバー / 近傍 / 非近傍 / `focusOnHover` off の 4 分岐と、ズームのランプが upstream のままであること）と、inline スクリプト側の構造（`label.alpha` の書き手が生成時の 1 件と `renderLabels()` だけであること・`renderNodes()` と同じ `active` を読むこと・`focusOnHover` で分岐すること・退避復元とズーム側の代入が消えたこと・係数 `3.75` の写しが残っていないこと）である。**遡及適用は行わない**（`quartz-plugins/` は `update: overwrite`）。
...
>
> **`content/sources/` の索引ページは本件では扱わない。** `_write_sources_index()` / `_write_source_dir_indexes()` も巨大ハブだが、(1) `showSources` トグルで丸ごと切れるため読者に逃げ道が既に在り、(2) 索引を外すと Issue #839 が導入した prune の入力が変わって**実測で決めた挙動が動く**。したがって「build 生成の索引ページはグラフに出さない」という一般則は**まだ立てない**（**この保留のうち entity 側は Issue #983 が解いた** — 型インデックスと言語ルートは既定で落ちる。`sources/` 側は上の 2 理由のまま据え置きであり、下記コールアウト参照）。
>
...
タグノードは設計上どの言語にも属さず（ページと翻訳が同じタグを共有する）、root・source ノードも同様である。したがって1言語に絞り込んだときにこれらが道連れで消えてはならない — **消してしまうと、言語クラスタを繋ぐ唯一の橋を、言語を見比べようとした瞬間に失う**。タグは `showTags` / `removeTags` キー、sources は `showSources` キーで別に切る。build 生成の索引ページ（型インデックスと言語ルート）だけは逆で、`showIndexes` が**既定で落とす**（Issue #983。下記）。

> **タグノードがノード集合へ入る経路は 2 本ある — 切るのはリンク構築ではなくノード集合の側である（Issue #982）**: この節はかつて「タグは**既存の** `showTags` キー … で別に切る」と書き、フォーク側のコメントも「`showTags` は upstream がリンク構築で読むので、このトグルは自前のフィルタコードを持たなくてよい」と明言していた。**その前提は成り立たない。**
>
> **Quartz はタグページを実ページとして `contentIndex.json` に書き出す**（`tags/agent-safety`・`tags/index` 等）。グローバルグラフのノード集合は `new Set(data.keys())` から作られるため、**タグノードは `showTags` と無関係にそこへ入る**。`showTags` が制御していたのはもう 1 本の経路（ページ → タグの疑似リンクを積むか）だけだったので、off にすると**エッジだけが消えてページノードが残る** — `wikicommit/ai-driven-dev-wiki` の実測（2026-09-21）で 969 ノード中 **123 件がタグ**、しかも**全件が `links: []`** であり、キャンバスの約 13% がリンクを 1 本も持たない点になっていた。`removeTags` に名前を挙げたタグにも同じことが起きる。
>
> **Issue #839 の prune が拾わなかった理由は 2 つあり、片方だけでは直らない**。(1) タグを off にしても `filterNodes()` は 1 件も落とさないので `narrowed` が偽になり、次数境界も既定 0 なので**早期 return に入って prune 自体が走らない**。(2) 走ったとしても `pageDegreesBefore` は**既にタグリンクが抜かれた `links`** から取られるため、タグは「前」も 0 になり、「元から孤立しているノードを巻き添えにしない」という**正しいガードが逆に働く**。
>
> したがって **`showTags` / `removeTags` を `GraphFilterConfig` へ移し、`filterNodes()` が `kind === "tag"` のノードを落とす**。フォーク自身が「絞り込みのチョークポイントは `neighbourhood` 確定後・`nodes` 構築前の 1 箇所に集約されている」と書いており、そこに寄せるのが本来の形である。`showSources` と同じ機構になり、隣り合う 2 つのトグルが別々の仕組みで動く非対称も消える。
>
> **リンク構築側は 2 分岐になり、両者は同じ規則の言い換えではない**。**グローバルグラフ（`depth < 0`）では疑似リンクを無条件に構築する** — そうしないと上記 (2) の baseline 汚染が残る。**ローカルグラフ（`depth >= 0`）では従来どおり `showTags` / `removeTags` で切る** — あちらでは `filterNodes()` が呼ばれない（BFS の経路を断つため。下記「適用範囲はグローバルグラフのみ」）ので、無条件に積むと Tags を off にしてもタグノードへ辿り着いてしまう。**同じ `links` 配列を両分岐が読むため、一方の都合で無条件化すると他方が壊れる。**
>
> **`removeTags` の照合は大文字小文字を無視する**。`removeTags` はページの `tags:` frontmatter の綴りを指す一方、公開されるタグページの slug は Quartz のスラッグ化で小文字になる（Issue #981 が explorer で踏んだのと同じ小文字化）。大小だけが違う 2 つのタグはどのみち 1 ページに解決されるため、畳んでも元から別だったノードを併合することにはならない。
>
> **採らなかった案**: `validLinks` からタグを除外する（`showTags: true` のとき、どのページからも参照されていないタグページが二度と描かれなくなる — 孤立していること自体が報告に値する情報であり、Issue #340 / #547 の規則と正面から衝突する）／`minDegree: 1` を既定にする（下記のとおり既に退けている。元からの孤立とフィルタ由来の孤立を混ぜる）／prune の条件から `pageDegreesBefore > 0` を外す（(1) の早期 return が残るうえ、Issue #839 が明示的に置いたガードを外すことになる）。
>
> **ローカルグラフに同じ症状は出ないはずだが未実測**（BFS は `links` を辿るので、`showTags: false` ならタグノードへ到達しない）。**upstream にも同じ欠陥があるかは確認していない** — ノード集合の構築も `showTags` の意味づけも upstream のものであり同じ症状が出ると見込まれるが、本件はフォーク側で閉じる。**遡及適用は行わない**（`quartz-plugins/` は `update: overwrite` なので、再 init または `/wikicommit-update` を実行したリポジトリから届く）。

**ただし、そのフィルタが繋がりを奪った tag / source ノードは落とす（Issue #839）**。リンクは両端が残っている場合にしか描かれないため、残したこれらのノードは**リンクを 1 本も持たない点**になる。`wikicommit/ai-driven-dev-wiki` の規模（source ページだけで 100 件超）では型を 1 つ選ぶだけで画面の大半が孤立した点の雲になり、読者には「ノードが隠れた」ではなく**「リンクが切れた」**と映る。**この 2 つは矛盾しない** — 上の規則が守っているのは「まだ見えているページに繋がっているタグ」であり、それは Issue #584 が守ろうとしたものそのものである。2 言語にまたがるタグは 1 言語に絞っても**残る側の言語のページに繋がったまま**なので、この prune では決して消えない。消えるのは「選ばれた型・言語のページを 1 つも持たないタグ」だけである。

判定は「**フィルタ後にページへ届く本数が 0 で、かつフィルタ前は 0 ではない**」の 1 つである。

**素の次数では答えられない**（実装時に判明し、素の次数版は source ノードに対して 1 度も発火しないことを実測で確認した）。公開される source ツリーは内部でリンクし合っている — `convert_wikilinks.py` の `_write_sources_index()` が全 source ページを `content/sources/index.md` から、`_write_source_dir_indexes()` がディレクトリ索引から再度リンクし、`generate_root_index()` が `sources` 自身をリンクする。これらの隣接ノードは**すべて `kind === "source"`** であり言語・型フィルタを素通りするため、**どれだけ絞り込んでも全 source ノードの次数は 1 以上のまま**になる。つまり本 Issue が対象とする 100 件超の点の雲にこそ効かない。そこで prune が数えるのは「**entity ノードへ届くリンクの本数**」に限る — これは本節自身の一文（「タグとソースは、ページを介してしか存在しない」）を測れる形にしたものである。次数境界（`minDegree` / `maxDegree`）の側は従来どおり素の次数を使う — あちらはコントロールバーの "Links per node" であり、読者に見えているリンクすべてを意味するためである。

この 1 つの判定が 3 つを同時に満たす:

- **孤立ページ・未使用ソースを隠さない**。元からページへ届かないノード（1 ページも生まなかった `status: failed` / `excluded` のソース、orphan ページ）はフィルタ前も 0 なので対象外である。孤立は WikiCommit が `check_orphans.py` で**報告する対象**（Issue #340 / #547）であり、グラフがそれを黙って隠す層になってはならない
...
> **型インデックスと言語ルートは既定で落とす — `isIndex` に初めて消費者を与えた（Issue #983）**: `NodeInfo.isIndex` は Issue #584 の当初から `<lang>/<Type>/index.md` の型インデックスと `<lang>` の言語ルートに立ち、コメントは "navigation, not an entity page" とまで書いていた。**ところがそれを読む主体がどこにも無かった** — `filterNodes()` は一度も参照せず、inline スクリプト側の出現回数は 0 だった。Issue #553 が繰り返し禁じている「受け皿だけ存在して常に空のまま残る」の、フラグ側での現れである。
>
> **結果として、型インデックスは普通のページとしてグラフに入り、巨大ハブになっていた。** `wikicommit/ai-driven-dev-wiki` の公開 `contentIndex.json` の実測（2026-09-21）で `en/definedterm` の発リンクは **214 本**、これは `en` 配下の実コンテンツページ 483 件の 44% に 1 個のノードが触れていることを意味する。型インデックス ＋ 言語ルートの計 11 ノードで 483 本。build 生成の索引ページ全体（型インデックス・言語ルート・`sources/` の索引群・`overview/`）を起点とするエッジは 3,763 本中 1,160 本（31%）を占める。**Issue #584 が潰そうとした状態そのもの**である。
>
> **`check_orphans.py` との食い違いも同時に解消する。** あちらは孤立判定にあたって `index.md` を**リンク元として明示的に除外する**（Issue #678） — インデックスの `- [[Type/slug]]` は `rebuild_index.py` が全ページに対して機械的に張るものであり、「誰かがこのページを参照した」証拠ではないためである。グラフ側にその除外が無い間、**`check_orphans.py` が孤立と報告するページをグラフは「リンクを持つページ」として描いていた**（同パイロットでの実測は 1 件。この Wiki は相互リンクが密なため規模は小さいが、向きは Issue #957 が俯瞰ページについて決め手にしたものと同じで、機械が生成したナビゲーションのリンクが WikiCommit 自身の報告を打ち消している）。
>
> **既定は非表示で、`showIndexes` が戻す。** `GraphFilterConfig` / `D3Config` の `showIndexes?: boolean` は**省略時 `false`** であり、`showSources` / `showTags`（省略時は表示）とは極性が逆である。したがって実装の綴りも `!== false` ではなく `=== true` になる。**コントロールバーには出さない** — バーは既に混んでおり（Issue #838）、凡例の形は 3 種で埋まっていて 4 つ目には新しい形の設計が要る（Issue #841）。露出は凡例トグルの Issue #985 に委ね、本件は「既定で落とす ＋ YAML で戻せる」に留める（**Issue #985 は既存 2 トグルと凡例の重複解消に範囲を絞り、索引トグルは足していない** — 露出は Issue #1018 に切り出した。上の Issue #985 のコールアウト参照）。`storeFilters()` が localStorage に書くキーは固定の 6 つであり `showIndexes` を含まないため、`quartz.config.yaml` で `true` にした値が保存済みフィルタのマージで潰れることもない。
>
> **root index（`/`）は対象外である。** 条件を `kind === "entity" && isIndex` としたのは、`classifyNode()` が root index を `kind: "root"` と分類しており、上のコールアウトが「発リンクは `<lang>/`・`sources`・`overview/` だけでハブ化もしなければ嘘もつかない」として明示的に残すと決めているためである。この条件なら、その決定に 1 文字も触れずに済む。**ただし root index の見え方は変わる** — `overview/` は Issue #957 が、`<lang>/` は本件が落とすため、残る発リンクは `sources` 1 本になり、`showSources` を off にすると root index は孤立点になる。`kind: "root"` は prune の対象（`tag` / `source`）ではないので消えはしないが、この帰結は記録しておく。
>
> **除外の実装位置は Issue #957 と同じ `all` の構築時点である。** `kept` から後で引くと `narrowed = kept.size !== all.size` が常に真になり、既定表示でも毎回 prune 経路へ入って `computeEntityDegrees()` の全走査が 2 回余計に走る（上のコールアウトが述べた早期 return を潰す）。
>
> **prune の挙動は動かない。** 型インデックスの発リンク先は entity ページだけであり、tag も source もインデックスをリンクしない（インデックスの frontmatter に `tags` は無く、source ページの `generated_pages` リンクは entity ページを指す）ため、**tag / source の entity 次数は 1 も変わらない**。インデックスを隣人として失う entity ページ側は、そもそも entity ノードが prune されない（「選んだ型に属するページがリンクを持たないことは、そのページを隠す理由にならない — それは孤立ページであり、まさに読者が見られるべきものである」）ので影響を受けない。**俯瞰ページのときと違い、この主張はテストで固定してある** — あちらは除外が無条件で 2 実装が戻り値から区別できなかったのに対し、こちらは `showIndexes` で両分岐に到達できるためである。
>
> **「Links per node」の数え方は変わる。** 次数境界が読むのは plain degree（`computeDegrees()`）であり、インデックスが消えた分だけ各エンティティページの次数が 1 前後下がる。既定は `minDegree: 0` / `maxDegree: 0` なので既定表示は動かないが、`minDegree` を立てている読者には効く — そしてこれは**数え方が正確になった側の変化**である。機械が全ページに自動で張る `- [[Type/slug]]` を「このページが持つリンク」として数えないことは、`check_orphans.py` が最初から採っている数え方と同じである。
>
> **単体テストの id は完全なエンティティパスで書く。** `classifyNode()` は**単一セグメントの id を言語ルートとみなす**ため、`"a"` / `"hub"` のような略記のフィクスチャは本件以降インデックスとして落ちる。実運用では `content/` 直下の単一セグメントは `<lang>` か予約接頭辞しか現れないので分類は正しく、直すべきはフィクスチャの側である（既存の次数境界テスト 3 件をこの形に書き換えた）。
>
> **孤立ページは孤立点として描かれる — これが意図した見え方である。** 型インデックスからのリンクしか持たないページは、本件以降グローバルグラフで辺を 1 本も持たない点になる。entity ノードは prune されない（Issue #839）ので消えはせず、`check_orphans.py` が孤立と呼ぶものと**グラフの見た目が初めて一致する**。元から孤立しているノードを機械が隠してはならない（Issue #340 / #547）という規律とも同じ向きである。
>
> **既知の限界: ローカルグラフには残る**（Issue #957 の俯瞰ページと同じ）。`graph.inline.ts` は `filterNodes()` を `depth < 0` でしか呼ばないため、あるページのローカルグラフにはその型インデックスが隣接ノードとして残る。ローカルグラフは「このページの近傍」を見る別の面であり、型インデックスはそこでは実際に辿れる隣人であるため、受け入れる。
>
> **採らなかった案**: `kind: "index"` を新設する（`collectFacets()` が `kind !== "entity"` を弾くため型インデックスの言語・型がファセットから消える。既存の `isIndex` に消費者を与えるほうが小さく、しかも本件が指摘した欠陥そのものを閉じる）／`maxDegree` で隠す（214 を切る上限は正当なハブページも巻き込むうえ、既定 `maxDegree: 0` では効かないので既定表示が変わらない）／`rebuild_index.py` が `- [[Type/slug]]` を書くのをやめる（インデックスは読者が型ごとにページを辿るための実ページであり、グラフの都合で内容を変えるのは筋が違う。**直すべきはグラフ側の読み方である**）／Issue #957 と同じ無条件除外にする（俯瞰ページは「トグルの on 側に価値が無い」ため無条件で足りたが、型インデックスは読者が実際に辿る普通のページであり、その繋がりを見たい運用者がいてもおかしくない）。
>
> **遡及適用は行わない**（新旧混在を許容する方針）。`quartz-plugins/` は `update: overwrite` なので、変更は再 init または `/wikicommit-update` を実行したリポジトリから届く。**ブラウザでの目視確認も行っていない**（Issue #81。上と同じ）。
...
#### コントロールバーは面を持ち、トグルは塗りで状態を示す（Issue #840）
...
Sources / Tags のトグルは**素のチェックボックスから `aria-pressed` を持つ 2 状態のボタンへ**変える。on と off の差が「チェックマークの有無」から**塗りの有無**という面積のある差になり、バーの `font-size: 0.8rem` でも読める。Reset と同じ見た目の語彙になるため、バー全体が 1 つのコントロール群として読める点も効く。ネイティブの `<button>` にするのは、Tab・Space・Enter とフォーカスリングを自前で実装せずに保つためである（チェックボックスが無償で与えていたもの）。`buildToggle()` が返す `{field, sync}` の契約は変えないため、**Issue #651 の「バーを作り直さず値だけ書き戻す」経路はこの変更を知らない**。
...
あわせて `select, input` の一括指定を `select, input[type="number"]` に限定した。素の `input` はトグルのチェックボックスにも当たっていたが、`appearance: none` が無い以上 `background-color` / `border` がそこで効くかはエンジン依存であり、**ブロックを読んでも適用されるのかどうか分からないこと自体**が見分けにくさの一因だった。
...
**落とし穴 2 つはそのまま残るが、「同じ矩形に合わせた独自の絶対配置」は無くなった（Issue #838）**。当初はバーとコンテナがどちらも `position: fixed` で、**同じ中央矩形に別々に手計算で合わせていた** — コンテナが `height: 80vh` / `top: 50%` / `translate(-50%,-50%)`、バーが `top: calc(50% - 40vh)` / `translate(-50%,-100%)` である。結果、バーが使える縦幅は上に空く **10vh に固定**され（ビューポート 700px なら 70px）、`size="4"` の複数選択とキャプションで ~90px になるため、`max-height` + `overflow-y: auto` で**バーの中をスクロール**させてしのいでいた。狭さは副作用ではなく設計値であり、キャップを外しても `.global-graph-outer` の `overflow: hidden`（`backdrop-filter` があるため、これが fixed な子の containing block でもある）に切られるだけで解決しない。

現在は `.global-graph-inner` というラッパーを 1 つ挟み、**矩形の定義を 1 箇所に寄せた**。`.global-graph-inner` が `position: fixed` で中央・`80vw × 80vh`・`display: flex; flex-direction: column` を持ち、バーは `flex: 0 0 auto`（内容なりの高さ）、コンテナは `flex: 1 1 auto; min-height: 0` で残りを取る。**`min-height: 0` は必須である** — flex アイテムの既定 `min-height: auto` は内容より縮まないため、バーが高くなるとコンテナがラッパーの下へはみ出す。

**バーの `max-height` / `overflow-y` は残す**（用途が変わった）。旧実装のそれは 10vh の帯に押し込むためのものだったが、現在は `max-height: 50%` + `overflow-y: auto` + `min-height: 0` を**裾の保険**として持つ — `flex-shrink: 0` である以上、ビューポートが低く（横向きのスマートフォン・高さを半分にしたウィンドウ・ページズーム 200%）`size="8"` のリストが 2 行に折り返してバーが 80vh を超えると、グラフが 0px に潰れ、バー下端の Reset ボタンが `.global-graph-outer` の `overflow: hidden` の外へ押し出されてどこからも触れなくなる。通常の縦幅では発動しない。
...
**クリック除外リストには `.global-graph-inner` も足した**（完了条件の判断）。バーとコンテナの隙間（ラッパーの `gap`）はモーダルの内側であり、そこを踏んで閉じるのは「あと 1 クリック足りない」種類の驚きになるため、**閉じない側に倒した**。モーダルを閉じる 3 経路（背景クリック・Esc・アイコン再クリック）はいずれも変わっていない — 背景は `.global-graph-outer` の余白であり、ラッパーの外側なので従来どおり閉じる。

あわせて複数選択リストの行数上限を `4` から `8` に上げた。型が 20 種ある Wiki を 4 行の窓で操作している状態だったが、**上げても意味が出るのはラッパー化の後である** — それ以前はキャップに当たってスクロールが増えるだけだった。**採らなかった案**: 数値をずらすだけ（コンテナを 72vh・キャップを 14vh に）は最小差分だが、同じ数字を 2 箇所に持つ構造こそが原因であり、次にコンテナの高さを触ったときに同じ形で再発する／バーをグラフの上にオーバーレイする（領域は取り放題だがグラフを隠す。グラフは俯瞰のための面なので本末転倒）／折りたたみにする（常用しない操作向けの形であり、フィルタは連続操作するもの — Issue #651 がフォーカスとスクロール位置の保持に手を入れたのはそのためである）。
...
| 知識の中心 | 被リンク数ランキング（ハブページ Top 20）。各行に**そのページに記録されているソースの件数**を添え、一覧の直前に何を数えた値かを述べる 1 文を置く（Issue #990）。一覧の末尾に、同じ 1 本のソースだけに立つハブが 2 件以上あればそのソースと件数を出す（Issue #1006） |
| 型別の傾向 | 型ごとの件数・レビュー済み率・平均被リンク数・孤立件数 |
| 知識の不足 | wanted ページ（参照元件数順）・orphan ページ |
| 情報源の内訳 | 種別別・`status` 別・URL ホスト別・**抽出テキストの言語別**（Issue #989）の件数と、情報源の種別 × 生成されたページの型のクロス集計 |
| タグ | 頻度ランキング |
...
>
> **分母は翻訳ページを除いたページである（Issue #1030）**: 翻訳ページは `sources` を持たず（`translated_from` で継承する）、`wikicommit-translate` の品質チェックは記録されない（Issue #750 が意図的に対象外とした。`docs/DesignDoc-data.md` §4.8）。それを分母にだけ入れていたため、**原文を全部照合済みの Wiki が 1 言語に全訳した瞬間に 50%、2 言語なら 33% に下がって見えた** — 翻訳を足すという Wiki を良くする行為が、この数字の上では品質の低下として現れる。**分子も同じ条件で絞る** — 翻訳ページに `/wikicommit-review` を AI が走らせれば `kind: ai` の記録が書かれ、分母だけを絞ると割合が 100% を超えうる。
>
> **黙って外さない**。翻訳ページが 1 件以上あれば、その件数と「原文との照合は記録されていない」ことを述べる行を AI の行の直下に出し、補足文にも「翻訳ページはこの照合の対象外であり、上の割合にも含めていない」の 1 文を足す。外したことをその場で言わないと、読者は翻訳ページも照合済みの割合に含まれていると読む。**翻訳ページが 0 件の Wiki では行も 1 文も出ない**（翻訳の無い Wiki に無関係な注記を増やさない）。総ページ数と「人が読んで確認」の分母は変えていない — 翻訳ページも人は読めるためである。
>
> **補足文の主語も直した**。旧文面の英語版は "Every page is checked against its own sources" であり、翻訳ページについて明確に偽だった。主語を「出典から生成されたページ」に限定しており、この 1 語は翻訳の有無に関わらず変わる。
>
> **言語別には分けない**。ルート index（Issue #769。上記 §8.8 のコールアウト）は同じ理由（翻訳ページは記録を持たず、サイト全体の数字が薄まる）で件数を言語別にしたが、あちらは言語ごとの入口という別の目的でそうなっている。ここで問題なのは言語ではなく「照合の記録を持ちえないページの種類」であり、判定の軸を言語に置くと、1 つの言語に原文と翻訳が混ざる構成で再び薄まる。`translated_from` の有無で分ければ構成によらず同じ答えになる。
>
> **view ページ（`derived_from`）は動かしていない**。現在は `synthesize-step5.5` の記録で「出典と照合」に数えられているが、照合の相手は外部のソースではなく Wiki 内の grounding ページである — 同じ過大表明の形だが、原因（記録の有無ではなくラベルの意味）が違うため本 Issue の対象外とし、事実としてここに記録する。**翻訳ページの照合そのものを記録することは Issue #1031 の範囲**であり、それが入れば翻訳ページの行は「原文と照合済み（AI）: N / M」に置き換わる。
>
> **指摘の種類別内訳と、公開されなかったページの件数（Issue #1063）**: `ai_findings` の直下に、種類ごとの件数を入れ子の箇条書きで出す。ラベルは内部の enum 名（`HALLUCINATION` 等）ではなく読者向けの言葉（「出典に書かれていない記述」「出典と食い違う記述」「手元に無い文書に依拠した記述」「その他」）にする。**母集団は `ai_findings` と同じ**で、`load_ai_review()` の戻り値を件数 1 つから種類別の件数へ広げたため、ルックアップは 1 回のまま、内訳の合計は構造上総数と一致する（合わないと読者はどちらかを誤りと読む）。enum 外の値と `type` の無い指摘は推測で振り分けず「その他」にまとめ（実測で約 1 割）、件数が 0 の種類は行ごと出さない。
>
> **`page_at_fault: other` は総数からも内訳からも外した**。これは「相手ページが誤っていそう」という非ブロッキングの報告（Issue #566）で、そのページで書き直されたものではないため、「指摘を受けて書き直された箇所」に数えるのは過大表明だった。判定は値が文字列 `other` と一致するかだけで、ドリフトした値（`this` / `self`。Issue #987）は「このページの指摘」として残す。**このため既存の Wiki では次のビルドから `ai_findings` の数字が下がる**。
>
> あわせて**「検査を通らず公開されなかったページ」の件数**を出す（0 件なら行ごと出さない）。数えるのは「最新の `kind: ai` 記録が `result: discarded` で、かつそのページがディスクに無い」ページで、entity と view の両方（synthesize Step 5.5 も破棄する）。**`action: update` の破棄は数えない** — 旧版が公開されたまま残り、「公開されなかった」が偽になる。後の実行で通ったページも、最新の記録が pass なので数えない（`check_review_coverage.py` の `latest_discarded()` は「最新の discarded 記録」を返すので流用できず、`latest_by_kind()` で最新の AI 記録そのものが discarded かを見る）。**ページ名は出さない** — 公開しないと決めた内容の見出しだけを公開することになるため。補足文は件数が Wiki の欠陥ではなく検査が働いた記録であることを述べる。
>
> **走査を 1 つ足した — 上の「既存の走査に相乗りする」原則の例外である**。ディスクに無いページの記録は全ページ走査からは届かないため、`count_unpublished_pages()` が `.wikicommit/review/` を 1 回歩く。記録数に比例するコストで、ディスクに在るページのディレクトリは記録を読む前に飛ばす。root index（Issue #769）には出さない — 入口の数字は 3 つに留める。
...
> **ソースの言語別を出す（Issue #989）**: `## 情報源の内訳` の 5 つ目の小節として、管理ファイルの `source.lang`（Pass 2a が書く ISO 639-1。`docs/DesignDoc-data.md` §4.3）を数える 2 列の表を置く。集計は `generate_source_pages()` の `stats`（`lang_counts`）に相乗りし、追加の走査は無い。**ホスト別の表では代替できない** — ホスト名から言語は導けず、しかもホスト数は青天井で `OVERVIEW_HOST_LIMIT` の裾に非英語ホストが集中する（`wikicommit/ai-driven-dev-wiki` の 2026-09-21 公開サイトで 66 ホスト中 19 が非英語、うち 11 が切り落とされていた）。言語数は有界なので、この表には件数上限を置かない。
>
> **未記録（値を持たない管理ファイル）は必ず最後の 1 行として出す**。フィールドは遡及付与されず、再実行でも `status: generated` のソースには付かない。隠すと、英語ソースだらけの Wiki が記録済みの少数の言語だけでできているように見え、本節が見せようとしているものと正反対の嘘をつく。表の直前に 1 文を置き、`primary_lang` 以外の行が出るのは正常であり、そのソースから書かれたページが要約・翻訳を経ていることを意味する、と述べる（`reviewed_note` / `ai_reviewed_note` と同じく、数字が誤読されうる箇所に 1 文を添える慣習）。コードは言語名へローカライズしない。
>
> **ハブ行にソースの件数を添える（Issue #990）**: `ORPHAN:` 行は Issue #570 で出自を添え、「**端が薄い**」（どのソースが 1 ページしか作らなかったか）を見えるようにした。一方「**中心が薄い**」 — 最も参照されているページが 1 本の文書にしか立っていないこと — を見ている主体は 1 つも無かった。ハブ行は `— 被リンク数: 24 / 情報源: 2` の形になる。値は `page_stats` の `source_count` で、**既に読んでいる `sources[]` の長さから導く**（`has_manual_source` の導出と同じ読み取りに相乗りし、追加の I/O を発生させない）。
>
> **実測 — 指標は生きている**（キー単位。`assets/` / `index.md` / `status: removed` を除外し、`sources[]` は `primary_lang` のページから取った）:
>
...
>
> **測定に使ったクローンは公開サイトより古いスナップショットである**（Issue #989 が 2026-09-21 に公開サイトを測った時点で既に大きい）。示しているのは絶対値ではなく関係の形である。
>
> **非ハブは両 Wiki とも 100% 単一ソースである** — 2 件目の `sources[]` エントリが付くのは `action: update` のときだけで、2 本のソースが同じ実体に触れた時点でそのページは参照されやすくなる。**この数字が情報を持つのはハブの行だけ**であり、置き場所を Knowledge hubs に限った決定の根拠そのものである。そして指標は 2 つの Wiki を弁別する（decameron の上位ハブは 20 件中 18 件が複数ソース、ai-driven-dev はそうではない）。
>
> **規範は「多ソースにせよ」ではなく「単一ソースのハブが見えない状態であってはならない」に置く**。正当な単一ソースのハブが 2 種類ある — source-as-entity ページ（Issue #475。そのページ自身がその文書についてのページなので構造上 1 件）と、1 本の論文が造語した用語（正直な出典が 1 本しか無い）。ai-driven-dev の被リンク最多ページ `ScholarlyArticle/agentic-software-engineering-foundational-pillars`（31 件）が前者にあたる。したがって補足文（`sources_count_note`）は (a) 記録されているソースの件数であって独立した裏付けの数ではないこと、(b) ある文書そのものについてのページは構造上 1 件になること、を述べる。**単一ソースを太字・記号で強調しない** — 1 であることが正しい場合がある以上、強調は判定になる。
>
> | 論点 | 決定 | 理由 |
> |---|---|---|
...
> | view ページ | 区画ごと省く | `derived_from` であって `sources` ではない（Issue #675）。件数を同じラベルで出すと別のものを数えることになる |
> | `sources[].type: manual` | 数に含める | それはソースである |
> | `status: retracted` のソース（Issue #737） | 引かない | 引くには管理ファイルを読む必要があり、hub 集計に `.wikicommit/source/` への依存を持ち込む。取り下げ済みソースに立つページは `check_retracted_sources.py` が運用者へ報告している |
> | source-as-entity ページ | フィルタしない | Issue #475 はマーカーを付けなかったので機械的に判定できない。型からの推定はヒューリスティックであって事実ではない。補足文で扱う |
> | `/wikicommit-status` | 足さない | 全キーの 88% / 83% が単一ソースであり、所見にすると常時点灯する（Issue #562 / #864 / #867） |
> | ページ単位のバナー | 出さない | Issue #751 の線引き（ページ単位の数字は判定に読まれる。件数が意味を持つのは集約された後） |
> | 型別の傾向に平均情報源数の列 | 足さない | 非ハブが 100% 単一ソースである以上、型全体の平均は「その型に何件ハブがあるか」を薄めて映すだけで、型そのものの性質を述べない |
>
> **ラベルのキーは `sources`（`## 情報源の内訳` の見出し）と分け、`sources_count` / `sources_count_note` を新設した**（Issue #664 / #751 が `reviewed` と `ai_reviewed` について確立した規則）。英語ラベルも見出しの `Sources` と文字列を変えて `Sources recorded` にしてある。
>
> **per-row の件数では集中は見えない**。ai-driven-dev の上位 20 ハブの単一ソース 12 件のうち **7 件が同じ 1 本の arXiv 論文**（`arxiv.org/pdf/2509.06216`。単独で 31 ページを生成）に由来しており、この Wiki の概念的中心が 1 本の論文が造語した語彙で占められている — Issue #430 / #451(3) がページ単位で扱った偏りの、リンクグラフ単位での再現である。集中の可視化は別 Issue とした（per-row の件数が入った後であれば独立に足せる。逆は成り立たない）— 下の Issue #1006 の段落が入れたものである。
>
> **遡及は不要** — このページは build-generated なので次のデプロイで全件に反映される。同じ件数は `build_survey_view.py` の `PAGE:` 行にも `sources=N` として出し、`/wikicommit-collect` Step 3.5 が「何が薄く支えられているか」を着眼点の材料にできるようにした（`docs/DesignDoc-ScriptSpec.md`）。
>
> **同じソースに立つハブをまとめて出す（Issue #1006）**: 上の段落が別 Issue に送った集中の可視化である。Knowledge hubs 節の**末尾**（ハブ一覧の直後）に、「上位 20 件のうち、次の情報源だけに立つもの:」に続けて 1 ソース 1 行で `[<識別子>](<ソースページ>) — N 件` を出す。**新しい節は作らない**（俯瞰ページの目次を伸ばさない）。**判定ではなく報告である** — `sources_count_note` と同じ強さでその旨を直前の 1 文に書き、正当に集中する 2 つの形（ある文書そのものについてのページ〈Issue #475〉・1 つの文書を主題とする Wiki）を併記する。
>
> | 論点 | 決定 | 理由 |
> |---|---|---|
...
> | 数えるもの | **単一ソースのハブだけ**を、その唯一のソースの識別子でまとめ、**2 件以上を占めるソースだけ**を出す | 実測の「12 件中 7 件」と同じ量になる。複数ソースのハブまで数えると Wiki の主要記事（decameron の `it.wikipedia.org/wiki/Decameron` 等）が健全な Wiki でも点灯し、弁別性を失う見込みが高い（**未計測** — 含めたくなったら先に decameron で測ること） |
> | 分母から外すもの | 翻訳ページ・view ページ（`source_count` が `None` のもの） | 識別子を持たない。行ごとの件数と同じ扱い |
> | 識別子 | `sources[].url` / `sources[].path` の文字列そのもの（Issue #572 / #573） | 導出ファイル名は旧命名規則の管理ファイルを取りこぼす。ホスト単位にしない理由は上の表と同じ |
> | 「どの N 件か」 | 行に印を付けず、`content/sources/` のソースページへリンクする（`generated_pages` を列挙しているので辿れる）。ソースページが無い識別子は素のテキスト | 行の強調は判定になる（上の段落） |
> | リンクの引き方 | `generate_source_pages()` が返す `source_stats` に `page_for_source`（`source.url` / `source.path` → 出力パス）を足す | 同関数は全管理ファイルを既に走査しており、追加 I/O が無い |
> | 該当なし | 行ごと出さない（「なし」とも書かない） | 集中していないことは報告すべき事実ではない |
> | `status: retracted` | 引かない | 上の表と同じ |
> | `/wikicommit-status` | **足さない** | 上の表の「常時点灯する」は**ここでは使えない**（集中は普遍的ではない — 上位 20 のうち単一ソースは decameron 2 件・ai-driven 12 件）。根拠は別に 2 つある: (1) これは運用者がページ単位で直す欠陥ではなく Wiki の現在の形についての事実であり、status の所見は「これを直せ」と読まれるため、source-as-entity や 1 文書の Wiki で誤った行動を促す、(2) 集中を解く行動（別の情報源を探す）は `/wikicommit-collect` の俯瞰（Step 3.5）の領分である |
>
> `page_stats` はソースの**件数**しか持っていなかったので、同じ `sources[]` の読み取りから識別子（`source_ids`）も持たせた — 追加の I/O は無い。**指標が生きていることの唯一の実例は上の実測（ai-driven-dev の上位 20 ハブのうち単一ソース 12 件中 7 件が同じ 1 本の論文）である。** 遡及は不要（次のデプロイで出る）。
...
>
> **その `View` の段は、本番で 1 度も発火していなかった（Issue #981）**。下の図は Issue #946 が確定させたものだが、**実装されていたのは `overview` と `sources` の 2 段だけ**で、`View` は tier 0 に落ちて Type フォルダの列の末尾（`displayName` の `View` によるアルファベット順）に埋もれていた。`wikicommit/ai-driven-dev-wiki` の実測で判明した。
>
> **原因は、予約ツリー 3 つで綴りの由来が違うことにある。** 比較先は publish 後の slug であり、Quartz は各セグメントを小文字化する（`convert_wikilinks.py` の `_quartz_slugify_segment()` が移植している規則。末尾が `.lower()`）。公開サイトの `contentIndex.json` がそのまま証拠で、`filePath: "en/View/index.md"` に対する slug は `"en/view/index"` である。
>
> | 予約ツリー | 名前の出どころ | 元の綴り | slug | Issue #946 の判定 |
> |---|---|---|---|---|
> | `sources` | `SOURCES_DIR_NAME`（ディレクトリ名定数） | 小文字 | `sources` | ⭕ |
> | `overview` | `OVERVIEW_DIR_NAME`（同上） | 小文字 | `overview` | ⭕ |
> | `View` | `VIEW_TYPE_SEGMENT`（**予約 Type セグメント**） | PascalCase | `view` | ❌ |
>
> 3 つのうち 2 つは「Python 側で小文字のディレクトリ名として決めた」ものなので**たまたま正しく**、1 つは「Type セグメントの命名規約（PascalCase）に従う」ものなので外れた。**同じ種類の識別子として扱えたのは「予約されている」ことだけで、綴りの由来は違う。**
>
> **テストが緑のままだったのは、ノードを手で作っていたからである。** `WikiCommitExplorer.test.tsx` は `folder("View")` と大文字でノードを作り、Issue #946 が足した 3 件が揃ってその綴りを共有していた。`tests/test_explorer_sort_tier_sync.py` は 2 つの写しの一致しか見ないので、**両方が同じように誤っていることは検出できない**（これは同テストの設計どおりで欠陥ではない）。現在は `sortTier` の冒頭にこの不変条件をコメントとして書き、テストのノードを `folder("view", "View")`（`displayName` は実物どおり `"View"` のまま）に直し、`contentIndex` の実データの形から slug を導くテストを 1 件足してある。`tests/test_publish_reserved_trees.py` も `VIEW_TYPE_SEGMENT.lower()` が両方の写しにリテラルとして現れることを見る（grep 相当であり、tier の正しさは見ない）。
>
> **`RESERVED_PUBLISH_TREES` に `View` を足す形は採らない** — あれは `convert_wikilinks.py` が `content/` 直下に予約するツリーの一覧であり、`View` は `content/<lang>/` の下にある別のものである。**`.toLowerCase()` も付けない**（`slugSegment` が大文字になる経路は無く、隣の 2 行と書き方が割れる）。**`displayName` での判定も採らない** — 本コールアウトが `overview` について退けた当のものであり、`View` の `title` が今たまたま英語リテラルであることに依存することになる。
>
> 確定した並びは次のとおり。
>
> ```text
...
再検討の時期は「将来 Phase」と漠然と書いていたが、`qmd` はローカル LLM 推論を前提とするツールであり、これは Phase 4 の Tauri デスクトップアプリにおけるローカル LLM(Ollama 等)対応 — ユーザー自身の LLM をローカルで動かす実行系が必要であり、**その対応自体が未決定である**（当時の計画は `dev/roadmap-phase4-5.md` にある。同ファイルは公開対象外 — `docs/README.md` §3）— と同じ前提(ローカル推論基盤)を必要とする。したがって両者は独立に検討するのではなく、Phase 4 でローカル LLM 対応に着手するなら、その基盤の上で `qmd` 採用も併せて再検討するのが筋が良い。Phase 4 でローカル LLM 対応自体を見送るなら、`qmd` 採用の前提も同時に崩れるため、こちらも見送りが妥当。
