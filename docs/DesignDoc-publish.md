...
**808行の `graph.inline.ts` には D3 force simulation と PixiJS の描画本体がある。フィルタの動的化はそこに一切触れずに実現できた**（描画本体への変更は後から 1 件だけ入った — 下記 Issue #841）。動的化に必要な土台が既に揃っていたため:
...
上流からの変更はソース中に `WikiCommit:` として印を付けてある。**当初の 4 箇所はいずれも描画本体の外側**（設定の読み取り・`neighbourhood` の絞り込み・コントロールバーの構築・クリック除外リスト）であり、その後 2 件が加わった: 凡例（5）と、**ノードの形による種別の区別（6。Issue #841）— これが描画本体への唯一の変更である**。
